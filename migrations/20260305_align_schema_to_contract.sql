-- Destructive migration diff: aligns existing schema to the v1 contract.
-- Review before running in non-dev environments.
...
DROP TABLE IF EXISTS budget_profiles;
...
-- Recreate schema aligned with openapi.yaml and api_v1.md.
...
CREATE TABLE transactions (
  ...
  import_fingerprint CHAR(64) NULL COMMENT 'sha256(date|amount|lower(trim(expense))|category|tag|card)',
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  deleted_at DATETIME NULL,
  PRIMARY KEY (id),
