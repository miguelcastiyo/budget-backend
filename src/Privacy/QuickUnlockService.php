...
final class QuickUnlockService
{
    ...
    private function binary(mixed $value,string $field,int $min,int $max): string { if(!is_string($value)||$value==='')throw new HttpException(422,'VALIDATION_ERROR',$field.' is required');$v=base64_decode(strtr($value,'-_','+/').'==',true);if($v===false||strlen($v)<$min||strlen($v)>$max)throw new HttpException(422,'VALIDATION_ERROR','Invalid '.$field);return$v; }
